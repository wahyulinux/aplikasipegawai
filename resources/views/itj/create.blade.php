...
<div class="max-w-2xl mx-auto bg-white p-10 rounded-2xl shadow-2xl border border-gray-100 mt-10">
    <div class="mb-8 border-b pb-4">
        <h1 class="text-3xl font-extrabold text-gray-900">Input Work Order Tarik Jalur</h1>
        <p class="text-sm text-gray-500 mt-2">Tarif ITJ saat ini: <span class="font-bold text-blue-600">Rp {{ number_format($defaultNominal, 0, ',', '.') }}</span></p>
    </div>
    
    <form action="{{ route('itj.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Pilih Pegawai Pelaksana (Tanpa Batas Maksimal)</label>
        ...
        <div>
            ...
            <textarea name="keterangan" rows="2" placeholder="Detail tarikan jalur..."
        ...
        <div class="bg-blue-50 p-4 rounded-xl border border-blue-100">
            <p class="text-xs text-blue-800 leading-relaxed">
                <span class="font-bold">Sistem Bagi Rata:</span> Nominal WO sebesar <strong>Rp {{ number_format($defaultNominal, 0, ',', '.') }}</strong> akan otomatis dibagi rata berdasarkan jumlah pegawai yang dipilih di atas tanpa ada batasan jumlah pekerja.
            </p>
        </div>

        <div class="pt-6 flex items-center justify-between">
            <a href="{{ route('itj.index') }}" class="text-gray-500 hover:text-gray-800 font-bold transition">Batal</a>
