    ...
    <div class="overflow-x-auto">
        <table class="min-w-full">
            <thead class="bg-gray-50 text-gray-600 uppercase text-xs font-bold tracking-wider">
                <tr>
                    @if(Auth::user()->role !== 'pegawai')
                        <th class="py-4 px-6 text-left whitespace-nowrap">Pegawai</th>
                    @endif
                    <th class="py-4 px-6 text-left whitespace-nowrap">Nominal Pinjam</th>
                    <th class="py-4 px-6 text-center whitespace-nowrap">Tenor</th>
            ...
            <tbody class="text-gray-600 text-sm">
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                    ...
                    @endif
                    <td class="py-4 px-6 font-semibold whitespace-nowrap">Rp {{ number_format($loan->nominal_pinjaman, 0, ',', '.') }}</td>
                    <td class="py-4 px-6 text-center whitespace-nowrap">{{ $loan->tenor_bulan }} Bln</td>
                    <td class="py-4 px-6 text-right text-red-600 font-bold whitespace-nowrap">Rp {{ number_format($loan->nominal_cicilan, 0, ',', '.') }}</td>
                    <td class="py-4 px-6 text-right font-black text-blue-700 whitespace-nowrap">Rp {{ number_format($loan->sisa_pinjaman, 0, ',', '.') }}</td>
