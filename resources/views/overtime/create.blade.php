@extends('layouts.app')

@section('content')
<div class="max-w-2xl mx-auto bg-white p-10 rounded-2xl shadow-2xl border border-gray-100 mt-10">
    <div class="mb-8 border-b pb-4">
        <h1 class="text-3xl font-extrabold text-gray-900">Input Data Lembur</h1>
        <p class="text-sm text-gray-500 mt-2">Tarif Lembur per Orang: <span class="font-bold text-green-600">Rp {{ number_format($defaultNominal, 0, ',', '.') }}</span></p>
    </div>
    
    <form action="{{ route('overtime.store') }}" method="POST" class="space-y-6">
        @csrf
        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Tanggal Lembur</label>
            <input type="date" name="tanggal" required value="{{ date('Y-m-d') }}"
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition">
        </div>

        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Pilih Pegawai Lembur</label>
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto">
                @foreach($employees as $emp)
                    <label class="flex items-center space-x-3 p-2 hover:bg-white rounded-lg transition cursor-pointer border border-transparent hover:border-blue-200">
                        <input type="checkbox" name="employee_ids[]" value="{{ $emp->id }}" 
                            class="employee-checkbox w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        <div class="text-sm">
                            <p class="font-bold text-gray-800">{{ $emp->nama }}</p>
                            <p class="text-xs text-gray-500">{{ $emp->jabatan }}</p>
                        </div>
                    </label>
                @endforeach
            </div>
            @error('employee_ids')
                <p class="text-xs text-red-500 font-bold mt-1">{{ $message }}</p>
            @enderror
            <p id="counter-msg" class="text-[10px] text-gray-500 mt-2 italic">Terpilih: 0 Orang</p>
        </div>

        <div>
            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wide">Keterangan Tambahan</label>
            <textarea name="keterangan" rows="2" placeholder="Tugas lembur yang dikerjakan..."
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition"></textarea>
        </div>

        <div class="bg-blue-50 p-4 rounded-xl border border-blue-100">
            <p class="text-xs text-blue-800 leading-relaxed">
                <span class="font-bold">Sistem Fixed Rate:</span> Setiap pegawai yang dicentang di atas akan masing-masing mendapatkan <strong>Rp {{ number_format($defaultNominal, 0, ',', '.') }}</strong> yang akan otomatis ditambahkan ke slip gaji bulan ini.
            </p>
        </div>

        <div class="pt-6 flex items-center justify-between">
            <a href="{{ route('overtime.index') }}" class="text-gray-500 hover:text-gray-800 font-bold transition">Batal</a>
            <button type="submit" id="submit-btn" class="bg-blue-600 text-white px-8 py-3 rounded-xl hover:bg-blue-700 font-black shadow-lg transform hover:-translate-y-1 transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                Simpan Data Lembur
            </button>
        </div>
    </form>
</div>

<script>
document.querySelectorAll('.employee-checkbox').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
        const checkedCount = document.querySelectorAll('.employee-checkbox:checked').length;
        document.getElementById('counter-msg').innerText = `Terpilih: ${checkedCount} Orang`;
        document.getElementById('submit-btn').disabled = (checkedCount === 0);
    });
});
document.getElementById('submit-btn').disabled = true;
</script>
@endsection
